...
async function main() {
  try {
    ...
    try {
      ...
      console.error(
        "Error processing the response or generating the phrase:",
        error
      );

      // Fallback phrase
      const fallbackPhrase = "Have a nice day!";
      console.log("Using fallback phrase:", fallbackPhrase);

      // Call the typewriter effect function with the fallback
      typeWriterEffect(fallbackPhrase, "speech-bubble-text");
    }
  } catch (error) {
